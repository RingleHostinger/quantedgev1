...
import { BarChart2, Lock, Star, Clock } from 'lucide-react'
...
function HeatCard({ row, rank }: { row: HeatmapRow; rank: number }) {
  ...
  // Which team does the model prefer (higher win prob)?
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="rounded-xl px-3 py-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div
              ...
              style={{
                background: modelPrefersHome ? heat.bg : 'rgba(255,255,255,0.03)',
                border: `1px solid ${modelPrefersHome ? heat.border : 'rgba(255,255,255,0.06)'}`,
              }}
            >
              <div className="text-xs mb-0.5 truncate" style={{ color: '#A0A0B0' }}>{row.home_team}</div>
              <div
                className="text-base font-black"
                style={{ color: modelPrefersHome ? heat.color : '#E6E6FA' }}
              >
                {Math.round(homeProb)}%
              </div>
            </div>
        ...
        {/* Public betting — Coming Soon */}
        <div
          className="rounded-xl px-3 py-3 space-y-1.5"
          style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.1)' }}
        >
          <div className="flex justify-between items-center">
            <span className="text-xs font-semibold" style={{ color: '#4A4A60' }}>Public Bets %</span>
            <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(255,255,255,0.04)', color: '#4A4A60', border: '1px solid rgba(255,255,255,0.07)' }}>
              Coming Soon
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-xs font-semibold" style={{ color: '#4A4A60' }}>Market Alert</span>
            <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(255,255,255,0.04)', color: '#4A4A60', border: '1px solid rgba(255,255,255,0.07)' }}>
              Coming Soon
            </span>
          </div>
...
export default function HeatmapPage() {
  ...
  return (
    <div className="p-6 space-y-6" style={{ background: '#0F0F1A', minHeight: '100%' }}>
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mb-2" style={{ color: '#A0A0B0' }}>
            Live view of where the AI model disagrees with sportsbook lines. Sorted by edge magnitude.
          </p>
          {/* Timestamps */}
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5" style={{ color: '#4A4A60' }} />
              <span className="text-xs" style={{ color: '#4A4A60' }}>
                Model Updated: {formatTs(modelUpdatedAt)}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Edge strength legend + summary counts */}
      <div className="flex flex-wrap gap-3 items-center">
        <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#6B6B80' }}>Edge Strength:</span>
        {[
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-xs mt-2" style={{ color: '#6B6B80' }}>
            Check back as odds refresh and new games are added.
          </p>
        </div>
      ) : (
        <>
